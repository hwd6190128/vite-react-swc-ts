...
const useMathValidator = ({
                              ...
                              availableFunctions = [{ label: '', value: '' }],
    ...
    const validateExpression = useCallback((expr: string): ValidationResult => {
        ...
        // 1. 驗證大括號中的指標
        const indicatorErrors = validateIndicators(expr);
        errors.push(...indicatorErrors);

        // 2. 驗證函數調用（保持原邏輯）
        const functionErrors = validateFunctions(expr);
        errors.push(...functionErrors);

        // 3. 使用 MathJS 驗證數學表達式語法
        const mathErrors = validateMathExpressionWithMathJS(expr);
    ...
    }, [availableIndicators, availableFunctions]);

    // 1. 驗證大括號中的指標
    const validateIndicators = useCallback((expression: string): string[] => {
    ...
    const validateFunctions = useCallback((expression: string): string[] => {
        ...
        const isValidParameter = (param: string): boolean => {
            ...
            // 檢查是否為指標 {xxx}
            const indicatorMatch = trimmedParam.match(/^{([^}]+)}$/);
            if (indicatorMatch) {
    ...
    // 3. 使用 MathJS 驗證數學表達式語法
    const validateMathExpressionWithMathJS = useCallback((expression: string): string[] => {
    ...
    // 當 expression 改變時觸發 debounced 驗證
    useEffect(() => {
